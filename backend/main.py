...
async def tmdb_get(path:str, params:Dict[str,Any]) -> Dict[str, Any]:
    ...
    if response.status_code != 200:
        raise HTTPException(
            status_code = 502, 
            detail=f"TMDB API returned status code {response.status_code}: {response.text}: {response.text}"
        )
...
def build_title_to_idx_map(indices: Any) -> Dict[str, int]:
    ...
    try:
        ...
        raise RuntimeError(
            "indices.pkl must be dict or pandas Series like object with (idx, title) pairs. "
        )
...
def get_local_idx_by_title(title:str) -> int:
    ...
    if TITLE_TO_IDX is None:
        raise RuntimeError("TITLE_TO_IDX mapping not initialized")
    key = _norm_title(title)
    if key in TITLE_TO_IDX:
        return int(TITLE_TO_IDX[key])
    raise HTTPException(
        status_code=404,
        detail=f"Title not found local dataset: '{title}'"
    )
...
def tfidf_recommend_titles(
    ...
    if df is None or tfidf_matrix is None:
        raise HTTPException(
            status_code=500,
            detail="TF-IDF model not initialized"
        )
...
def load_models():
    ...
    if df is None or "title" not in df.columns:
        raise RuntimeError("DataFrame must be loaded with a 'title' column")
...
#tfidf recommendations(local) + genre recommendations(tmdb)
...
async def search_bundle(
    ...
    if not best:
        raise HTTPException(
            status_code=404,
            detail=f"No TMDB results found for query: '{query}'"
        )
    tmdb_id = int(best["id"])
    details = await tmdb_movie_details(tmdb_id)
